...
export const useAppQuery = ({ url, fetchInit = {}, reactQueryOptions }) => {
	const authenticatedFetch = useAppFetch(url, fetchInit);
	const fetch = useMemo(() => authenticatedFetch, [url, JSON.stringify(fetchInit)]);
	const queryKey = reactQueryOptions?.queryKey ?? [url];
	return useQuery({ queryKey, queryFn: fetch, refetchOnWindowFocus: false, ...reactQueryOptions });
};
